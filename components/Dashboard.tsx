...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const generatePDF = (autoPrint: boolean) => {
    ...
    printWindow.document.write(`<html><head><title>${autoPrint ? 'REKOD' : 'PREVIEW'} KEHADIRAN - ${formattedDate}</title><style>@media print { body { margin: 0; padding: 10mm; } @page { size: A4; margin: 0mm; } .grey-box { background-color: #d1d5db !important; -webkit-print-color-adjust: exact; } .dark-grey { background-color: #000 !important; color: #fff !important; } } body { font-family: 'Arial Narrow', Arial, sans-serif; color: #000; line-height: 1.2; font-size: 10pt; ${!autoPrint ? 'padding: 20px; max-width: 800px; margin: 0 auto; background: #f8fafc; box-shadow: 0 0 20px rgba(0,0,0,0.1);' : ''} } table { width: 100%; border-collapse: collapse; table-layout: fixed; margin-bottom: -1.5px; background: #fff; } th, td { border: 1.5px solid black; padding: 4px; } .grey-box { background-color: #d1d5db; font-weight: bold; text-align: center; } .data-cell { font-size: 9pt; font-weight: bold; text-transform: uppercase; } .header-title { text-align: center; font-size: 12pt; font-weight: bold; padding: 10px; border: 1.5px solid black; background: #fff; }</style></head><body><div class="header-title grey-box">REKOD KEHADIRAN KELAS KESENIAN</div><table><tr><td class="grey-box" style="width: 15%;">KUMPULAN</td><td style="width: 25%; text-align: center; font-weight: bold;">${groupLabel.toUpperCase()}</td><td class="grey-box" style="width: 15%;">TINGKATAN</td><td style="width: 15%; text-align: center; font-weight: bold;">${tingkatLabel.toUpperCase()}</td><td class="grey-box" style="width: 10%;">BILIK</td><td style="width: 20%; text-align: center; font-weight: bold;">${(selectedRoom || '-').toUpperCase()}</td></tr></table><table><thead><tr><th rowspan="2" class="grey-box" style="width: 5%;">Bil</th><th rowspan="2" class="grey-box" style="width: 45%;">Nama Murid</th><th rowspan="2" class="grey-box" style="width: 15%;">Ting</th><th colspan="2" class="grey-box" style="width: 15%;">Tarikh / Masa<br>${formattedDate} / ${selectedTime}</th><th rowspan="2" class="grey-box" style="width: 20%;">Laporan Disiplin / Catatan</th></tr><tr><th class="grey-box" style="font-size: 7pt;">Jantina</th><th class="grey-box" style="font-size: 7pt;">Kehadiran</th></tr></thead><tbody>${tableRows}</tbody></table><table><tr><td class="grey-box" style="width: 20%;">Nama Jurulatih</td><td style="width: 40%; font-weight: bold;">${selectedCoach || '-'}</td><td class="grey-box" style="width: 20%;">Tandatangan Jurulatih</td><td style="width: 20%;"></td></tr><tr><td class="grey-box">Kelas lewat di keluarkan</td><td colspan="3"></td></tr><tr><td class="grey-box dark-grey">Disemak Oleh: Ketua Bidang</td><td class="dark-grey"></td><td class="grey-box dark-grey">Tarikh</td><td class="dark-grey"></td></tr><tr><td class="grey-box" style="text-align: left; padding-left: 10px;">MURID LELAKI</td><td colspan="3" style="font-weight: bold;">${maleCount} ORANG</td></tr><tr><td class="grey-box" style="text-align: left; padding-left: 10px;">MURID PEREMPUAN</td><td colspan="3" style="font-weight: bold;">${femaleCount} ORANG</td></tr></table>${scriptTag}</body></html>`);
        ...
        <div className="lg:col-span-2 bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
          ...
          <div className="mt-6 pt-4 border-t border-slate-100 flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 sm:flex sm:justify-end gap-2">
              <button onClick={handlePreviewPDF} className="w-full sm:w-auto px-5 py-3 bg-blue-100 text-blue-700 hover:bg-blue-200 font-black rounded-xl text-[10px] uppercase tracking-wider flex items-center justify-center gap-2 transition-colors"><i className="fas fa-eye"></i>PREVIEW</button>
