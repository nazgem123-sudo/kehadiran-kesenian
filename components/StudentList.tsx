          ...
          <table className="w-full">
            <thead className="bg-slate-50 text-slate-500 text-[10px] uppercase font-bold tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4 text-left">Tingkatan</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.length > 0 ? filtered.map((student) => (
                <tr key={student.id} className="hover:bg-blue-50/30 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-xs text-indigo-600 font-bold bg-indigo-50 px-2 py-1 rounded-md">{student.form}</span>
                  </td>
                  <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-xl rounded-3xl shadow-2xl overflow-hidden border border-slate-200 animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleEditSubmit} className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Tingkatan</label>
                  <select
                    ...
                    onChange={(e) => setEditingStudent({ ...editingStudent, form: e.target.value as Form })}
                  >
                    {FORMS.map(f => <option key={f} value={f}>{f}</option>)}
                  </select>
                </div>
